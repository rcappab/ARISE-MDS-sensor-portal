const timezonesWithoffsets =  Intl.supportedValuesOf('timeZone').map(timeZone => {
    let offset=new Intl.DateTimeFormat('en-GB',{timeZone:timeZone, timeZoneName:'longOffset'}).formatToParts().find(part => part.type==='timeZoneName').value.replace('GMT','UTC')
    let abbreviation = new Intl.DateTimeFormat('en-GB',{timeZone:timeZone, timeZoneName:'short'}).formatToParts().find(part => part.type==='timeZoneName').value.replace('GMT','UTC')
...
const browserTimezone = new Intl.DateTimeFormat('en-GB',{timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone, timeZoneName:'short'}).formatToParts().find(part => part.type==='timeZoneName').value.replace('GMT','UTC')
const browserTimezoneOffset = new Intl.DateTimeFormat('en-GB',{timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone, timeZoneName:'longOffset'}).formatToParts().find(part => part.type==='timeZoneName').value.replace('GMT','UTC')
...
function getTimeZoneCode(datetime){
    return datetime.toLocaleDateString('en-GB', {day:'2-digit', timeZoneName: 'short' }).substring(4);
}
